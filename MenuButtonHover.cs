       ...
       // [SerializeField] public int sphereKitchen;
        ...
        public void HandleSelectionComplete()
        {
            ...
            if(m_GazeOver)
               ...
               StartCoroutine(GoTo(nextSphere));
            GoToDone = true;
        ...
        void Update()
        {
            ...
            //if (basement.x > 5.8f)
            //{
            //Debug.Log(exit + "Princess Got It");
            //}
        ...
        IEnumerator GoTo(Transform nextSphere)
        {
           

            // Debug.Log(Camera.main.transform.parent.position + "Im in the basement");
            Camera.main.transform.parent.position = nextSphere.position;
           exit = nextSphere.position;
            //Debug.Log("First Room?");
            if (!finalRoom.visitedAll)
            {
                starScript.isFirstRoom = true;
            }
                
            // if (m_ShowLiving != null)
            //   m_ShowLiving.CheckedItems();
            //showMenu.SetActive(true);
